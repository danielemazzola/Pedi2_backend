...
const crearSalaComensales = async (req, res) => {
  ...
  const existe = await Mesas.find().where("nMesa").equals(usuario.mesa);
...
const mostrarComensales = async (req, res) => {
  const rest = await MesaComensales.find().where("restauranteId").equals(req.params.id)
  res.json({rest})
...
const eliminarComensal = async (req, res) => {
  ...
  const mesaComensal = await Mesas.find()
    .where("nMesa")
    .equals(comensal.mesa);
